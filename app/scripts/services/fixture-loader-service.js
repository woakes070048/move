  ...
  .service('fixtureLoaderService', function ($q, $http, locationService, facilityFactory, $rootScope, memoryStorageService, config, storageService, utility, pouchDB, syncService, ehaRetriable) {
    ...
    var loadDatabasesIntoMemoryStorage = function (databases) {
      if (Object.prototype.toString.call(databases) !== '[object Object]') {
        throw new Error('databases should be nested object.')
      }
    ...
    function loadFilesIntoCache (fileNames) {
    ...
    // The cache is used since this can be
    // called quite often on navigation
    // the keys is an array of db's stringified.
    // It's a little rough, but a simple way of making sure
    // all the dbNames asked for have been loaded
    var cache = {}
